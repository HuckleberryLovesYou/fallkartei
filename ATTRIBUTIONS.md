# Quellen, Namensrechte und externe Inhalte

## Projektstatus

Die Fallkartei ist ein inoffizielles, nicht-kommerzielles Fanprojekt. Es besteht keine Verbindung zu Sony Music Entertainment, EUROPA, dem KOSMOS Verlag oder weiteren beteiligten Rechteinhabern.

## Marken und geschützte Inhalte

„Die drei ???“, zugehörige Namen, Marken, Titel, Cover, Illustrationen, Hörspielinhalte, Figuren und sonstige geschützte Bestandteile gehören den jeweiligen Rechteinhabern.

Die Fallkartei nutzt diese Bezeichnungen ausschließlich beschreibend, um die verwalteten Hörspielfolgen zuzuordnen. Es wird kein Anspruch auf Rechte an diesen Inhalten erhoben.

## Metadaten

Ergänzende Folgeninformationen werden aus öffentlich verfügbaren Quellen zusammengeführt. Dazu gehören insbesondere:

- [dreimetadaten.de](https://dreimetadaten.de/) für ergänzende Metadaten, Rollen, Kapitel sowie Cover- und Streaminglinks
- [rocky-beach.com](https://www.rocky-beach.com/) für Community-Wertungen, soweit diese im Projektkatalog hinterlegt sind

Die Daten können unvollständig, veraltet oder fehlerhaft sein. Die Fallkartei garantiert keine dauerhafte Verfügbarkeit oder Richtigkeit externer Informationen.

## Cover

Coverdateien werden nicht als vollständiger Bildbestand in diesem Repository gespeichert. Die App lädt Cover optional über externe Links, die in den Metadaten hinterlegt sind. Wenn eine Bildquelle nicht erreichbar ist, wird ein neutraler Platzhalter angezeigt.

Alle Rechte an den angezeigten Covern und Illustrationen verbleiben bei den jeweiligen Rechteinhabern. Die externe Einbindung stellt keine Lizenzübertragung dar.

## Streamingdienste

Markennamen und Links zu Spotify, Apple Music, BookBeat, Amazon Music, YouTube Music, Deezer und Amazon dienen ausschließlich als Verweise auf externe Angebote. Die Fallkartei steht in keiner geschäftlichen Verbindung zu diesen Diensten.

## Eigenes Projektmaterial

Der Name **Die Fallkartei**, das App-Interface, der selbst erstellte Quellcode und das für dieses Projekt erstellte App-Icon sind eigenständige Projektbestandteile. Für den Quellcode wird derzeit keine Open-Source-Lizenz erteilt.


## Ergänzungen ab 1.4.0

- Metadaten zu den eigenständigen Live-Geschichten wurden anhand offizieller Serien-/Produktinformationen sowie der jeweiligen Streaming-Veröffentlichungen ergänzt.
- Bei Specials ohne vorhandenes Cover kann die App das Artwork zur Laufzeit über die öffentliche Apple-iTunes-Lookup-Schnittstelle aus dem bereits hinterlegten Apple-Music-Eintrag auflösen.
- Die App speichert diese externen Cover nicht im Repository, sondern nur als lokale URL-Zuordnung im Browser-Cache.
- Die Verbindungsdaten der Smart Playlist beschreiben redaktionell hinterlegte Handlungsstränge, wiederkehrende Figuren und dokumentierte Rückbezüge. Sie sind keine automatische Inhaltsanalyse.
