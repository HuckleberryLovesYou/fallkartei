...
const symbols = { minus:'★',neutral:'★★',plus:'★★★',super:'★★★★' };
...
const RATING_LEVELS = { minus: 1, neutral: 2, plus: 3, super: 4 };
const RATING_KEYS = ['minus', 'neutral', 'plus', 'super'];
function ratingButtons(nr,current,compact=false) {
  const currentLevel=RATING_LEVELS[current]||0;
  const starsHtml=RATING_KEYS.map((key,index)=>{
    const level=index+1;
    const isFilled=level<=currentLevel;
    return `<button type="button" class="star-btn ${isFilled?'filled':'empty'}" data-action="rate" data-nr="${nr}" data-rating="${key}" data-level="${level}" aria-label="${RATING_LABELS[key]}" title="${RATING_LABELS[key]}"><svg class="star-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg></button>`;
  }).join('');
  return `<div class="star-rating-widget ${compact?'compact':''}" data-nr="${nr}" data-current-level="${currentLevel}"><div class="star-rating-bar" role="radiogroup" aria-label="Bewertung">${starsHtml}</div></div>`;
}
...
function renderActiveFilters() {
  const chips=[]; if (appState.filter!=='all') chips.push(['filter',appState.filter==='rated'?'Bewertet':appState.filter==='unheard'?'Offen':appState.filter==='heard'?'Gehört':appState.filter==='notes'?'Mit Notiz':appState.filter==='reheard'?'Mehrfach gehört':'Status']); if (appState.authorFilter!=='all') chips.push(['author',appState.authorFilter]); if (appState.eraFilter!=='all') chips.push(['era',appState.eraFilter]); if (appState.yearFilter!=='all') chips.push(['year',appState.yearFilter]); if (appState.search) chips.push(['search',`„${appState.search}“`]);
...
function renderRanking() {
  ...
  if (appState.ranking==='rocky') {
    const list=appState.catalog.filter((episode)=>Number.isFinite(episode.rockyRanking)).sort((a,b)=>a.rockyRanking-b.rockyRanking);
    const header=`<div class="ranking-header"><span>Platz</span><span>Folge</span><span>Ø Note</span></div>`;
    html=header+list.map((episode,index)=>`<article class="ranking-row"><span>${index+1}</span><button data-open-episode="${episode.nr}"><strong>${esc(episodeTitle(episode))}</strong><small>${metaLine(episode)}</small></button><span class="ranking-value">${episode.rockyRanking.toFixed(2)}</span></article>`).join('');
    ...
    const header=list.length?`<div class="ranking-header"><span>Platz</span><span>Folge</span><span>Match</span></div>`:'';
    html=list.length?(header+list.map((entry,index)=>{const match=matchPresentation(entry.episode,profile,entry.score);return`<article class="ranking-row"><span>${index+1}</span><button data-open-episode="${entry.episode.nr}"><strong>${episodeTitle(entry.episode)}</strong><small>${match.reasons.map(esc).join(' · ')||metaLine(entry.episode)}</small></button><span class="ranking-value">${match.scoreValue}</span></article>`;}).join('')):'<div class="info-card">Keine ungehörten Folgen verfügbar.</div>';
  }
...
function streamingSectionHtml(episode) {
  ...
  return `<div class="streaming-primary">${preferred?`<a class="button primary full" href="${esc(preferred.url)}" target="_blank" rel="noopener" data-action="play-episode" data-nr="${episode.nr}">▶ Bei ${esc(preferred.label)} anhören</a>`:''}</div>${others.length?`<details class="provider-details"><summary><span>Weitere Anbieter</span><small>${others.length} verfügbar</small><b>＋</b></summary><div class="provider-grid">${others.map((entry)=>`<a href="${esc(entry.url)}" target="_blank" rel="noopener" data-action="play-episode" data-nr="${episode.nr}"><strong>${esc(entry.label)}</strong><span>Öffnen</span></a>`).join('')}</div></details>`:''}`;
}
...
function catalogStatusHtml() {
  ...
  return `<div class="settings-status-head"><span class="settings-status-dot ${online?'good':'neutral'}" aria-hidden="true"></span><div><strong>${online?'Katalog bereit':'Offline · lokaler Katalog aktiv'}</strong><small>${appState.metadataUpdatedAt?`Online-Metadaten ${esc(formatRelativeDate(appState.metadataUpdatedAt))}`:'Noch kein Online-Metadatenupdate gespeichert'}</small></div></div><div class="settings-status-grid"><div><span>Einträge</span><strong>${appState.catalog.length}</strong></div><div><span>Für 100 %</span><strong>${released}</strong></div><div><span>Cover</span><strong>${covers}</strong></div><div><span>Vorschau</span><strong>${future+placeholders}</strong></div></div><p>${future} angekündigt · ${placeholders} Platzhalter · ${extras} Zusatzinhalte außerhalb des normalen 100%-Zählers.</p>`;
}
...
function bindDelegatedEvents() {
  document.addEventListener('click',async(event)=>{
    ...
    switch(action.dataset.action){
      case'play-episode':{if(nr&&!statusOf(nr).heard){setHeard(nr,true);toast('Als gehört markiert.');refreshViews(nr);}break;}
