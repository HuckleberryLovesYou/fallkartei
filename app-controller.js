...
async function profileImageBlob() {
  ...
  ctx.fillStyle='#f7f8fa';ctx.font='850 54px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif';ctx.fillText('DIE FALLKARTEI',76,168);ctx.fillStyle='#9fa7b3';ctx.font='600 22px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif';ctx.fillText('Persönliches Hörprofil für Die drei ???',78,208);
...
async function shareProfileImage() {
  try {
    ...
    if(file&&navigator.share){
      ...
      if(supported){
        try { await navigator.share({title:'Mein Hörprofil – Die Fallkartei',text:'So sieht mein persönliches Hörprofil für Die drei ??? aus.',files:[file]}); return; }
...
function bindDelegatedEvents() {
  document.addEventListener('click',async(event)=>{
    ...
    switch(action.dataset.action){
      ...
      case'share-profile':await shareProfileImage();break;case'open-quick-rate':closeDialog('profileDialog');openQuickRate();break;case'open-my-ratings':appState.ranking='mine';renderRanking();closeDialog('profileDialog');navigate('ranking',{restore:false});break;case'rate-focus':$('episodeDialogBody').querySelector('.rating-buttons')?.scrollIntoView({behavior:'smooth',block:'center'});break;case'new-playlist-with':openPlaylistEditor(null,nr);break;
