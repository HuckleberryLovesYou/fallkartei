...
function payload() { return { app:'die-fallkartei',version:APP_VERSION,exportedAt:nowIso(),data:appState.user }; }
export async function exportBackup({forceDownload=false} = {}) {
  const body = JSON.stringify(payload(),null,2); const filename = `fallkartei-backup-${new Date().toISOString().slice(0,10)}.json`; const blob = new Blob([body],{type:'application/json'}); const file = typeof File === 'function' ? new File([blob],filename,{type:'application/json'}) : blob; let shared = false;
  if (!forceDownload && navigator.share && navigator.canShare?.({files:[file]})) {
    try { await navigator.share({title:'Die Fallkartei – Backup',files:[file]}); shared = true; } catch (error) { if (error?.name !== 'AbortError') console.warn(error); }
  }
